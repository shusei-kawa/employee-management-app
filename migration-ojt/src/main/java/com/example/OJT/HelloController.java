...
//// =========================================================
//}
//// ⑦ ユーザー操作によって画面からリアルタイムに給与計算を実行する処理（クラスの内側に収めました）
//@PostMapping("/salary/calculate/{employeeId}")
//public String calculateSalary(@PathVariable("employeeId") Integer employeeId,
//                              @RequestParam("targetMonth") String targetMonth,
//                              @RequestParam("absentDays") int absentDays,
//                              @RequestParam("lateHours") double lateHours) {
//
//    Employee employee = repository.findById(employeeId)
//            .orElseThrow(() -> new IllegalArgumentException("Invalid employee Id:" + employeeId));
//
//    // リアルタイム給与計算を実行してDBに保存
//    salaryService.executeCalculation(employee, targetMonth, absentDays, lateHours);
//
//    return "redirect:/salary/" + employeeId;
//}
//} // ← クラスを閉じるカッコは一番最後に記述します
...
public class HelloController {
    ...
    // ⑥ 指定された従業員の給与明細一覧画面を表示する処理
    @GetMapping("/salary/{employeeId}")
    public String showSalaryDetail(@PathVariable("employeeId") Integer employeeId, Model model) {
        Employee employee = repository.findById(employeeId)
                .orElseThrow(() -> new IllegalArgumentException("Invalid employee Id:" + employeeId));

        // バッチまたは画面から計算・保存された給与計算結果を取得
        List<MonthlySalaryResult> salaryList = salaryRepository.findByEmployeeId(employeeId);
